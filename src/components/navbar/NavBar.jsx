...
const BasicExample = () => {
  ...
  const [menu_class, setMenuClass] = useState("app__navbar-links hidden")
  const [open, setopen] = useState("openIcon open")
  
  const updateMenu = () => {
    if(!isMenuClicked) {
        setMenuClass("app__navbar-links visible")
        setopen("openIcon close")
    
    }
    else {
        setMenuClass("app__navbar-links hidden")
        setopen("openIcon open")
    }
    setIsMenuClicked(!isMenuClicked)
  }
  return (
    <nav style={{backgroundColor:window.scrollY >= 100?"red":""}} className="app__navbar">
